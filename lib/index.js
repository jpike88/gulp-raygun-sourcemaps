...
module.exports.upload = function upload(options) {
                ...
                function (files, callback) {
                    async.eachSeries(
                        ...
                        function (item, cb) {
                            ...
                            request.post({
                                url: 'https://app.raygun.com/upload/jssymbols/' + options.app + '?authToken=' + options.key,
                                formData: formData
    ...
    function recursiveWalk(base, dir) {
        var results = [];
        var basePlusDir = path.join(base, dir);
        var list = fs.readdirSync(basePlusDir);
        list.forEach(function(file) {
            var filePlusPath = path.join(basePlusDir, file);
            var filePlusDir = path.join(dir, file);
            var stat = fs.statSync(filePlusPath);
            if (stat && stat.isDirectory()) {
                results = results.concat(recursiveWalk(base, filePlusDir))
            } else {
                results.push(filePlusDir)
            }
        });
        return results;
    }
    ...
    function getFiles(directory, callback) {
        ...
        if (options.recursive) {
            files = recursiveWalk(path.join(options.root, directory), '');
        } else {
            files = fs.readdirSync(path.join(options.root, directory));
        }
